						...
						<div>
							<List
								...
								<ListItem
									...
									nestedItems={[
										<ListItem value={2} primaryText={`${order.description}`} />
									]}
								/>
								<Divider
									style={{
										...
										return (
											<ListItem
												...
												nestedItems={[
													<ListItem
														value={2}
														primaryText={`Contactpersoon: ${
															addres.contactPerson
														}`}
													/>,
													<ListItem
														value={2}
														primaryText={
															addres.email
																? `Email: ${addres.email}`
																: `Telefoonnummer: ${addres.telephoneNumber}`
														}
													/>,
													<ListItem
														value={2}
...
const mapStateToProps = function(state, props) {
	return {
		order:
			state.orders &&
			state.orders.find(
				order => `${order.orderNumber}` === props.match.params.orderNumber
			)
	}
}
